...
- `prompt`: Describe the image content including text, main elements, and structural relationships. Use the same language as the lecture notes output.
- **完整保留 MCP 返回的原始输出**，不要压缩、不要省略、不要总结
- 如果某张图片分析失败（重试3次后），该路径的描述为 `"[Image analysis failed]"`
...
- **重试机制**：调用 MCP 时，如果失败（超时或速率限制），自动重试3次，重试间隔 2s→4s→8s；如果仍失败，标记为 `[Image analysis failed]`
